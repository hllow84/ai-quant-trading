"""Merge per-year Dukascopy XAUUSD M1 bid + ask CSVs (2013-2017 backfill,
STATE_OF_PLAY Sec67 follow-up) into one clean spot dataset with a real
bid/ask spread column. Same method as scripts/merge_xauusd.py (which
built the 2018-2025 file) -- only the source dir/year range/output path
change.

Input : data/raw/xau_2013_2017/download/xauusd-m1-{bid,ask}-{year}.csv
Output: data/XAUUSD_M1_2013_2017_spot_dukascopy.csv
...
SRC = os.path.join(REPO, "data", "raw", "xau_2013_2017", "download")
OUT = os.path.join(REPO, "data", "XAUUSD_M1_2013_2017_spot_dukascopy.csv")
...
def main():
    ...
    if not (900 <= lo and hi <= 2200):
        print("WARNING: price range outside expected 2013-2017 spot gold band (~1050-1900).")
    else:
        print("OK: price range consistent with SPOT gold (not futures) for 2013-2017.")
