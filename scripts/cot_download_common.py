"""
cot_download_common.py — Shared puller for CFTC Commitment of Traders
(Legacy Futures-Only Combined) reports, any market.
...
publication lag any causal backtest MUST respect (handled downstream in each
signal script via a shift, not here).
...
Extracted from scripts/download_cot_gold.py (originally gold-only) so
scripts/download_cot_eur.py and any future instrument can reuse it without
duplicating the fetch/validate/save logic.
"""
...
def fetch(market_name: str, retries: int = 5) -> pd.DataFrame:
    params = (
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            rows = resp.json()
            if not rows:
                raise ValueError(f"empty response from CFTC Socrata endpoint for market={market_name!r}")
...
def fetch_and_save(market_name: str, out_path: Path, min_years: int = 15) -> pd.DataFrame:
    df = fetch(market_name)
    ...
    assert span_days > 365 * min_years, f"suspiciously short history: {span_days} days"
