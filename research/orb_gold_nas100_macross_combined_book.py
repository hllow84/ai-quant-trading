"""
orb_gold_nas100_macross_combined_book.py — ORB gold RETEST + NAS100
MACROSS combined book (§55), an eighth pairing, completing the "ORB gold
x both NAS100 candidates" set in parallel to §49/§54's "ORB gold x both
US30 candidates" set. Genuinely new combination not yet tested.
...
- NAS100 macross (§42): fast=10/slow=30/ema_trend=200/k_atr=1.0/R=4.0/
  H=192, Sharpe +0.963 standalone.

Quality gap ~1.55x (1.488/0.963) — very close to §54's ORB gold+US30
macross gap (~1.49x), which produced this project's lowest correlation
(+0.004) and a clean win under plain 50/50 with no risk-parity rescue
needed. Worth checking directly whether NAS100 macross behaves like its
sibling US30 macross when paired with gold, or differently — per §53's
own finding that the specific legs involved matter, not just family/
quality-gap size in the abstract.
...
book.py's run_orb_gold_leg() unchanged; NAS100 macross reuses strategies/
...
def run_nas100_macross_leg():
    ...
    print("--- NAS100 macross standalone (leg re-run, unchanged params) ---")
...
def main():
    ...
    nas_ret, nas_sh, nas_dd = run_nas100_macross_leg()
    ...
    print("=== Combined book: ORB gold RETEST + NAS100 macross ===")
    ...
    print(f"  NAS100 macross standalone: Sharpe {nas_sh:+.3f}, maxDD {nas_dd*100:.1f}%")
    ...
    out = RESULTS / "orb_gold_nas100_macross_combined_book.csv"
